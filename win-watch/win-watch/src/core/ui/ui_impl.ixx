...
namespace ui
{
	...
	bool c_window::render_window( char *name, float width, float height )
	{
		...
		while ( msg.message != WM_QUIT )
		{
			...
			const float target_frame_time = 1000.0f / 90.0f;
			if ( frame_time < target_frame_time )
			{
				Sleep( static_cast< DWORD >( target_frame_time - frame_time ) );
			}
		}
		...
		ImGui::Begin( "###main_painel", nullptr, flags );
		{
			...
			switch ( current_tab )
			{
				case tabs::TAB_PROCESSES:
				{
					...
					{
						for ( int i = 0; i < watcher_->processes.size( ); i++ )
						{
							if ( watcher_->processes.at( i ).pid == 0 )
								continue;
				...
				case tabs::TAB_SERVICES:
				{
					...
					{
						for ( int i = 0; i < watcher_->services.size( ); i++ )
						{
							if ( watcher_->services.at( i ).pid == 0 )
								continue;
